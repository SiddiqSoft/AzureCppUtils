...
def parse_cmake_file(filepath):
    ...
    for line in lines:
        ...
        if "MATCHES [Mm][Ss][Vv][Cc]" in stripped or "WIN32" in stripped:
            ...
            if "MATCHES" not in stripped and "WIN32" not in stripped:
                current_scope = "Core"
        ...
        if cpm_match:
            ...
            # Avoid duplicate entries for same package
...
def generate_markdown(dependencies, project_name="AzureCppUtils"):
    """
    ...
    # Always ensure Windows CryptoAPI is in platform deps for AzureCppUtils
    if not any(d["name"] == "Crypt32 / BCrypt" for d in dependencies):
...
def main():
    ...
    markdown_content = generate_markdown(all_deps, project_name="AzureCppUtils")
